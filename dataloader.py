...
def labels_change_color(label):
    ...
    return colors[torch.tensor(label).item()]
...
def get_data(path=""):
    ...
    points = np.array(pd.DataFrame(row_data.elements[0].data))  # 点云坐标和法向量
    faces = np.array(pd.DataFrame(row_data.elements[1].data))  # 面片索引加颜色
    n_face = faces.shape[0]  # 面片个数 # number of faces
    xyz = points[:, :3]  # coordinate of vertex shape=[N, 3] 点的坐标
    normal = points[:, 3:]  # normal of vertex shape=[N, 3] 点的法向量
    label_face = np.zeros([n_face, 1]).astype('int32')
    label_face_onehot = np.zeros([n_face, 8]).astype(('int32'))
    """ index of faces shape=[N, 3] """
    index_face = np.concatenate((faces[:, 0]), axis=0).reshape(n_face, 3)   # 面的索引
    """ RGB of faces shape=[N, 3] """
    RGB_face = faces[:, 1:4]     # RGB值
    ...
    normal_vertex = np.concatenate(
        (normal[index_face[:, 0], :], normal[index_face[:, 1], :], normal[index_face[:, 2], :]), axis=1)
    normal_face = faces[:, 5:8]
    curvatures_face = faces[:, 8:]
    ...
    points_face = np.concatenate((xyz_face, centre_face, normal_vertex, normal_face, curvatures_face), axis=1).astype('float32')
    ...
    for i, label in enumerate(labels):
        ...
        # label_face[np.logical_and.reduce(RGB_face == label, axis=1)] = i
        # label_face_onehot[np.logical_and.reduce(RGB_face == label, axis=1), i] = 1
...
def generate_plyfile(index_face, point_face, label_face, path=" "):
    ...
    with open(path, "a") as f:
        ...
        for i, data in enumerate(index_face):  # write new point index for every face
            RGB = labels_change_color(label_face[i, 0].item())  # Get RGB value according to face label
...
class plydataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # points_face = np.delete(points_face, -1, axis=1)
        ...
        curvature_mean = points_face[:, -1].mean()
        curvature_std = points_face[:, -1].std()
        points_face[:, -1] = (points_face[:, -1] - curvature_mean) / curvature_std
